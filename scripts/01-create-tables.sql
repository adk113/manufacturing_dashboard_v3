...
CREATE TABLE IF NOT EXISTS public.safety_events (
    ...
    severity VARCHAR(20),
    description TEXT,
    injured_person_id UUID REFERENCES public.employees(id) ON DELETE SET NULL,
    reported_by UUID REFERENCES public.employees(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_production_data_plant ON public.production_data(plant_id);
CREATE INDEX IF NOT EXISTS idx_five_s_audits_date ON public.five_s_audits(audit_date);
CREATE INDEX IF NOT EXISTS idx_five_s_audits_line ON public.five_s_audits(line_id);
...
CREATE INDEX IF NOT EXISTS idx_kaizen_ideas_status ON public.kaizen_ideas(status);
CREATE INDEX IF NOT EXISTS idx_kaizen_ideas_plant ON public.kaizen_ideas(plant_id);
...
-- Create RLS policies (allow all for authenticated users - customize as needed)
CREATE POLICY "Allow all operations for authenticated users" ON public.plants
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON public.production_lines
    FOR ALL USING (auth.role() = 'authenticated');
...
CREATE POLICY "Allow all operations for authenticated users" ON public.five_s_audits
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON public.kaizen_ideas
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON public.production_data
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON public.quality_inspections
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON public.resource_consumption
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON public.safety_events
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON public.environmental_data
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON public.employee_feedback
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON public.training_records
    FOR ALL USING (auth.role() = 'authenticated');
